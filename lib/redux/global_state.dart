...
import 'package:flutter_admin_app/redux/user_redux.dart';
...
GState appReducer(GState state, action) {
  return GState(
    ///通过 UserReducer 将 GState 内的 userInfo 和 action 关联在一起
    userInfo: UserReducer(state.userInfo, action),

    ///通过 ThemeDataReducer 将 GState 内的 themeData 和 action 关联在一起
    themeData: ThemeDataReducer(state.themeData, action),

    ///通过 LocaleReducer 将 GState 内的 locale 和 action 关联在一起
    locale: LocaleReducer(state.locale, action),
    login: LoginReducer(state.login, action),

    ///通过 GreyReducer 将 GState 内的 grey 和 action 关联在一起
    grey: GreyReducer(state.grey, action),
  );
}

final List<Middleware<GState>> middleware = [
  EpicMiddleware<GState>(loginEpic),
  EpicMiddleware<GState>(userInfoEpic),
  EpicMiddleware<GState>(oauthEpic),
  UserInfoMiddleware(),
  LoginMiddleware(),
];
